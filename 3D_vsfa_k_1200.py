...
if __name__ == "__main__":
    ...
    features_dir2 = "/cfs/cfs-3cab91f9f/liuzhang/open_datasets/konvid_features/" 
...
#     val_list =  val_list[0:10]
    ...
    train_dataset = VQADataset(features_dir,train_list, max_len=240,scale = 4.05)
...
#         break
    ...
    val_dataset = VQADataset(features_dir2, val_list, max_len=240,scale = 4.05)
...
#             sys.exit()
        ...
        if epoch % num_for_val ==0:
            ...
            if val_PLCC >best_PLCC:
                ...
                torch.save(state,os.path.join(trained_model_file,"covid_3D_Attention_{}_plcc:{}.pth".format(epoch+1,val_PLCC)))
                ...
                state = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
                torch.save(state,os.path.join(trained_model_file,"covid_3D_Attention_{}_epoch:{}.pth".format(epoch+1,epoch+1)))
                print("Epoch {} model saved!".format(epoch + 1))
