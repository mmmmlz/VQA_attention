...
if __name__ == "__main__":
    parser = ArgumentParser(description='"VQA')
    ...
    features_dir =  "/cfs/cfs-3cab91f9f/liuzhang/open_datasets/ft_local/live-qualcomm_val_features_O/"
    print("训练数据目录：",features_dir)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    features_dir2 = "/cfs/cfs-3cab91f9f/liuzhang/open_datasets/ft_local/live-qualcomm_val_features_O/"
...
#         val_data.append(val)
    ...
    # 训练数据集合
    videos_pic1 = []
    full_dataset = os.listdir(features_dir)     
    train_list, val_list = [],[]
    
    for i in range(len(full_dataset)):
        if i %4 ==0:
            val_list.append(full_dataset[i])
        else:
            train_list.append(full_dataset[i])
            
    
    #train_list, test_list = torch.utils.data.random_split(full_dataset, [4, 1])
...
#     val_list =  val_list[0:10]
    ...
    train_dataset = VQADataset(features_dir,train_list, max_len=240,scale =73.6)
...
#         break
    ...
    val_dataset = VQADataset(features_dir2, val_list, max_len=240,scale =73.6)
...
#             sys.exit()
        ...
        if epoch % num_for_val ==0:
            ...
            with torch.no_grad():
             ...
             #   y_test = np.zeros(len(result))
                ...
                for i, (features, length, label,name) in enumerate(tqdm(val_loader)):
                    ...
                    y_val[i] = 73.6*label.item()    
                    features = features.to(device).float()
                    label = label.to(device).float()
                    outputs = model(features)
                    
                    
                    y_pred[i] =73.6*outputs.item()
            ...
            if val_PLCC >best_PLCC:
                ...
                torch.save(state,os.path.join(trained_model_file,"LIVE_Q_3D_Attention2_{}_plcc:{}.pth".format(epoch+1,val_PLCC)))
                ...
                state = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
                torch.save(state,os.path.join(trained_model_file,"LIVE_Q_3D_Attention2_{}_epoch:{}.pth".format(epoch+1,epoch+1)))
                print("Epoch {} model saved!".format(epoch + 1))
