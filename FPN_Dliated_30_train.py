...
#         break
    ...
    model = FPN_Dliated_ResNet3D(Bottleneck, [3, 4, 6, 3]).to(device)  #
...
#             print("length",length)
        ...
        if epoch % num_for_val ==0:
            ...
            if val_PLCC >best_PLCC:
                ...
                torch.save(state,os.path.join(trained_model_file,"FPN_Dliated_ResNet3D_{}_plcc:{}.pth".format(epoch+1,val_PLCC)))
                ...
                state = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
                torch.save(state,os.path.join(trained_model_file,"FPN_Dliated_ResNet3D_{}_epoch:{}.pth".format(epoch+1,epoch+1)))
                print("Epoch {} model saved!".format(epoch + 1))
