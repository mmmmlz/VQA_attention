...
#     val_list =  val_list[0:10]
    ...
    train_dataset = FPN_VQADataset(features_dir,train_list, max_len,scale = 4.05)
...
#         break
    ...
    val_dataset = FPN_VQADataset(features_dir2, val_list, max_len,scale = 4.05)
    val_loader = torch.utils.data.DataLoader(dataset=val_dataset,)

    print("load val data success!")
    
    
    model = FPN_ResNet3D(Bottleneck, [3, 4, 6, 3]).to(device)  #
...
#         print("pre_modle:{}".format(path))
    ...
    for epoch in range(epoch_start,args.epochs):
        ...
        for i, (features,down_list,length, label,name) in enumerate(tqdm(train_loader)):
...
#             print("length",length)
            ...
            outputs = model([features,down_list])
        ...
        if epoch % num_for_val ==0:
            ...
            with torch.no_grad():
             ...
             #   y_test = np.zeros(len(result))
                L = 0
                for i, (features,down_list, length, label,name) in enumerate(tqdm(val_loader)):
            ...
            if val_PLCC >best_PLCC:
                ...
                torch.save(state,os.path.join(trained_model_file,"FPN3D_k_1200{}_plcc:{}.pth".format(epoch+1,val_PLCC)))
                ...
                state = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
                torch.save(state,os.path.join(trained_model_file,"FPN3D_k_1200{}_epoch:{}.pth".format(epoch+1,epoch+1)))
                print("Epoch {} model saved!".format(epoch + 1))
