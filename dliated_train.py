...
if __name__ == "__main__":
    ...
    features_dir = "/cfs/cfs-3cab91f9f/liuzhang/open_datasets/origin_k_1200/" 
    print("训练数据目录：",features_dir)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    features_dir2 = "/cfs/cfs-3cab91f9f/liuzhang/open_datasets/origin_k_1200/" 
...
#     val_list =  val_list[0:10]
    ...
    train_dataset = Dliated_VQADataset(features_dir,train_list, max_len=240,scale = 4.05)
...
#         break
    ...
    val_dataset = Dliated_VQADataset(features_dir2, val_list, max_len=240,scale = 4.05)
    val_loader = torch.utils.data.DataLoader(dataset=val_dataset,)

    print("load val data success!")
    
    
    model = Dliated_ResNet3D(Bottleneck, [3, 4, 6, 3]).to(device)  #
...
#             print("length",length)
            ...
            outputs = model(features,D_features)
        ...
        if epoch % num_for_val ==0:
            ...
            if val_PLCC >best_PLCC:
                ...
                torch.save(state,os.path.join(trained_model_file,"Dliated_3D_k_{}_plcc:{}.pth".format(epoch+1,val_PLCC)))
                ...
                state = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
                torch.save(state,os.path.join(trained_model_file,"Dliated_3D_k_{}_epoch:{}.pth".format(epoch+1,epoch+1)))
                print("Epoch {} model saved!".format(epoch + 1))
