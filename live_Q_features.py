...
#             self.videos_dir = "/cfs/cfs-3cab91f9f/liuzhang/video_data/video_clarity_vid"
      ...
      #  video_data = skvideo.io.vread(os.path.join(self.videos_dir, video_name))
        video_data = skvideo.io.vread(os.path.join(self.videos_dir, video_name), 600, 800, inputdict={'-pix_fmt':'yuvj420p'})
#         videogen = skvideo.io.vreader(os.path.join(self.videos_dir, video_name))
...
#     features_dir = "/cfs/cfs-3cab91f9f/liuzhang/video_data/CNN_features_mydata2/"   
    ...
    base_path = "/cfs/cfs-3cab91f9f/liuzhang/open_datasets/ft_local/live-qualcomm/"
    info_path= "./data/LIVE-Qualcomminfo.mat"
   # val_path ="/cfs/cfs-3cab91f9f/liuzhang/open_datasets/ft_local/cvd2014_split_label_val.pickle" 
    ...
    features_dir2 = "/cfs/cfs-3cab91f9f/liuzhang/open_datasets/ft_local/live-qualcomm_val_features_D/"
    
    
    Info = h5py.File(info_path, 'r')
    video_names = list([Info[Info['video_names'][0, :][i]][()].tobytes()[::2].decode() for i in range(len(Info['video_names'][0, :]))])
    scores = list(Info['scores'][0, :])
    video_name_score = []
    
    
    for vid ,score in zip(video_names,scores):
        #print(vid,score)
        name = vid+"--"+str(score)  
